  ...
  return (
    <div className="min-h-screen bg-[#200E3E]">
      <div className="flex h-full relative">
        <div className={`${
          isLeftBarVisible 
            ? "fixed inset-0 z-50 w-full h-full md:relative md:w-56" 
            : "hidden"
        }`}>
          <LeftBar 
            isVisible={isLeftBarVisible} 
            onClose={() => setIsLeftBarVisible(false)} 
          />
        </div>
        
        <div className={`w-full transition-all duration-300 ${
          isLeftBarVisible ? "md:ml-56" : ""
        }`}>
          <Navbar onMenuClick={() => setIsLeftBarVisible(prev => !prev)} />
