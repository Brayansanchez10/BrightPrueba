...
const DataTablete = () => {
    ...
    const fetchUserData = async () => {
      if (user && user.data && user.data.id) {
        try {
          ...
          console.error(
            "Error al obtener datos del usuario o permisos del rol:",
            error
          );
  ...
  const generateIds = () => {
    ...
    const entityFilteredCourses = searchFilteredCourses.filter((course) => {
      if (entityId === 1) return true;
      return course.entityId === entityId;
    });

    // Filtrado por userId solo si estamos en modo 'myCourses'
    const finalFilteredCourses = viewMode === 'myCourses'
      ? entityFilteredCourses.filter(course => course.userId === user?.data?.id)
      : entityFilteredCourses;

    // Paginar los cursos filtrados
    const paginatedCourses = finalFilteredCourses.slice(
      (currentPage - 1) * itemsPerPage,
      currentPage * itemsPerPage
    );

    // Generar los IDs para los cursos paginados
    return paginatedCourses.map(
  ...
  const filteredCourses = courses
    ...
    .filter((course) => {
      // Siempre filtrar por entityId (excepto para superadmin)
      if (entityId === 1) return true;
      return course.entityId === entityId;
    })
    .filter((course) => {
      // Filtrar por userId solo cuando estamos en modo 'myCourses'
      if (viewMode === 'myCourses') {
        return course.userId === user?.data?.id;
      }
      return true;
    });
  ...
  const canCreate = rolePermissions.some(
    (perm) => perm.nombre === "Crear Curso"
  );
  const canSection = rolePermissions.some(
    (perm) => perm.nombre === "Crear seccion"
  );
  const canContent = rolePermissions.some(
    (perm) => perm.nombre === "Asignar contenido"
  );
  const canEdit = rolePermissions.some(
    (perm) => perm.nombre === "Editar curso"
  );
  const canShow = rolePermissions.some((perm) => perm.nombre === "Ver curso");
  const canNotify = rolePermissions.some(
    (perm) => perm.nombre === "Notificar curso"
  );
  const canDelete = rolePermissions.some(
    (perm) => perm.nombre === "Eliminar curso"
  );
            ...
            <div className="px-4 md:px-12 -mb-1 ml-2 mr-2">
              <div className="flex space-x-1">
                <button
                  ...
                  className={`
                    px-6 py-2 rounded-t-lg font-bold
                    ${viewMode === 'myCourses' 
                      ? 'bg-secondaryAdmin text-primary border-2 border-b-0 border-cyan-200 dark:border-[#00d8a257]' 
                ...
                <button
                  onClick={() => setViewMode('allEntityCourses')}
                  className={`
                    px-6 py-2 rounded-t-lg font-bold
                    ${viewMode === 'allEntityCourses' 
                      ? 'bg-secondaryAdmin text-primary border-2 border-b-0 border-cyan-200 dark:border-[#00d8a257]' 
                      : 'bg-gray-200 text-gray-600'}
                  `}
                >
                  {t("courses.allEntityCourses")}
                </button>
              </div>
            </div>
                        ...
                        <tr key={course.id}>
                          ...
                          <td className="border-2 border-x-transparent px-6 py-2 bg-secondaryAdmin text-primary text-lg text-center border-t-transparent border-b-cyan-200 dark:border-b-[#00d8a257]">
                            <div className="flex justify-center space-x-4">
                              {(viewMode === 'myCourses' || course.userId === user?.data?.id || user?.data?.id === 1) ? (
                                <>
                                  <Button
                                  ...
                                  {/* Menú desplegable con todos los botones */}
                                ...
                                /* Solo mostrar el botón de detalles para cursos que no son míos */
                                canShow && (
                                  <Button
